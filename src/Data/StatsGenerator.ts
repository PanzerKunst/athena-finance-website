...
// curve is summed up at display time, restarting from 0 at the start of the window
...
function toPnlPointLines(stats: PeriodStats): string[] {
  return stats.buckets.map((bucket, index) => {
    const point = `{ date: "${toIsoDate(bucket.startedAt)}", label: "${formatBucketLabel(bucket)}", pnl: ${Math.round(bucket.pnl)} }`
